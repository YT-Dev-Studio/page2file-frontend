...
export const ExtensionGuide = ({ locale }: { locale: Locale }): ReactNode => {
  ...
  const step = (
    ...
    <article className={styles.step} key={item.number}>
      ...
      <div>
        <span className={styles.stepNumber}>{copy.stepLabel} {item.number}</span>
        <h2>{item.title}</h2>
        <p>{item.body}</p>
      </div>
    </article>
  ...
  const chapterItem = (
    chapter: (typeof copy.chapters)[number],
  ): ReactNode => (
    <li key={chapter.time}>
    ...
    <main className={styles.page} id="main-content">
      <Container>
        ...
        <header className={styles.hero}>
          <p className={styles.eyebrow}>{copy.eyebrow} · {locale.toUpperCase()}</p>
          <h1 className={styles.title}>{copy.title}</h1>
          <p className={styles.lead}>{copy.lead}</p>
        </header>
        <div aria-label={copy.formatLabel} className={styles.tabs} role="tablist">
          ...
          <div className={styles.videoGrid}>
            <div className={styles.videoCard}>
              <div className={styles.videoPoster}>
                <Image alt={copy.videoPosterAlt} height={720} src="/demos/video-poster.svg" width={1280} />
              </div>
